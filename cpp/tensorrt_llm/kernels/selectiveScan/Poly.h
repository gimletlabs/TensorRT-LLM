...
private:
    ...
    {
        return Poly<-bias_, decltype(-std::get<is_>(terms))...>{
            cn<-bias_>, std::tuple<decltype(-std::get<is_>(terms))...>{-std::get<is_>(terms)...}};
    }
...
__host__ __device__ Poly(Cn<value_>, Ranged<kinds_, sizes_, multipliers_>...)
    -> Poly<value_, Rn<kinds_, sizes_, multipliers_>...>;
