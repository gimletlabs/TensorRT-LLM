...
// quant_out: [M, K], Float8_e4m3fn - allocated quantized output tensor (F8E4M3FN)
// sf_out: ceil(M / 128) * 128 * ceil(K / sfVecSize / 4) * 4, SF_DTYPE allocated output scaling factors tensor (F8E8M0,
// encoded as SF_DTYPE [UINT8]).
void mxfp8_quantize_out(
    at::Tensor const& self, bool isSfSwizzledLayout, int64_t alignment, at::Tensor& quant_out, at::Tensor& sf_out)
{
    CHECK_TH_CUDA(self);
    CHECK_CONTIGUOUS(self);
    CHECK_TH_CUDA(quant_out);
    CHECK_CONTIGUOUS(quant_out);
    CHECK_TH_CUDA(sf_out);
    CHECK_CONTIGUOUS(sf_out);
...
#define LAUNCH_MXFP8_QUANTIZE_KERNEL(T)                                                                                \
    tensorrt_llm::kernels::invokeMxFP8Quantization(1, m, k, padded_k, reinterpret_cast<T*>(self.data_ptr()),           \
        reinterpret_cast<int64_t*>(quant_out.data_ptr()), reinterpret_cast<int32_t*>(sf_out.data_ptr()), layout,       \
    ...
    at::Tensor const& self, bool isSfSwizzledLayout, int64_t alignment = 32)
{
    ...
    mxfp8_quantize_out(self, isSfSwizzledLayout, alignment, valMxFP8, scaleFP8SF);
...
TORCH_LIBRARY_IMPL(trtllm, CUDA, m)
{
    ...
    m.impl("mxfp8_quantize_out", &torch_ext::mxfp8_quantize_out);
}
