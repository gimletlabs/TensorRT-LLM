...
def create_venv(project_dir: Path):
    ...
    print(f"-- Using virtual environment at: {venv_prefix} (Python {py_major}.{py_minor})")
    ...
    if not venv_prefix.exists():
        ...
        build_run(f'"{sys.executable}" -m virtualenv --system-site-packages "{venv_prefix}"')
    else:
...
def generate_fmha_cu(project_dir, venv_python):
    fmha_v2_cu_dir = (
        project_dir / "cpp/tensorrt_llm/kernels/contextFusedMultiHeadAttention/fmha_v2_cu"
    )
...
def create_cuda_stub_links(cuda_stub_dir: str, missing_libs: list[str]) -> str:
    ...
    for missing_lib in filter(lambda x: re.search(version_pattern, x), missing_libs):
        ...
        if so.exists():
            ...
            warnings.warn(f"Warning: Source library '{so}' does not exist and was skipped.")
...
def generate_python_stubs_linux(
    ...
    cuda_home_dir = (
        env_stub_gen.get("CUDA_HOME") or env_stub_gen.get("CUDA_PATH") or "/usr/local/cuda"
    )
    ...
    try:
        build_run(f'"{venv_python}" -m nanobind.stubgen -m bindings -r -O .', env=env_stub_gen)
        build_run(
...
def generate_python_stubs_windows(venv_python: Path, pkg_dir: Path, lib_dir: Path):
...
def build_kv_cache_manager_v2(project_dir, venv_python, use_mypyc=False):
    ...
    if use_mypyc:
        ...
        build_run(f'"{venv_python}" "{setup_mypyc}" build_ext --inplace', cwd=runtime_dir)

        # Verify that the shared library was generated
        if not list(runtime_dir.glob("*__mypyc*.so")):
            raise RuntimeError("Failed to build kv_cache_manager_v2: no shared library generated.")
        print("Done")
...
def main(
    *,
    ...
    venv_python, venv_conan = setup_venv(project_dir, project_dir / requirements_filename, no_venv)
    ...
    bolt_enabled = any("ENABLE_BOLT_COMPATIBLE=ON" in var for var in extra_cmake_vars)
    if bolt_enabled:
        if not nvrtc_dynamic_linking:
            cmake_def_args.append("-DNVRTC_DYNAMIC_LINKING=ON")
            print("-- BOLT: Forcing NVRTC_DYNAMIC_LINKING=ON (static NVIDIA libs lack relocations)")
    ...
    if cpp_only:
        ...
        targets.extend(["th_common", "bindings", "deep_ep", "deep_gemm", "pg_utils", "flash_mla"])
        build_pyt = "ON"
        build_deep_ep = "ON"
        build_deep_gemm = "ON"
        build_flash_mla = "ON"
    ...
    with working_directory(build_dir):
        if clean or first_build or configure_cmake:
            ...
            cmake_def_args.append(f"-DCMAKE_TOOLCHAIN_FILE={build_dir}/conan/conan_toolchain.cmake")
    ...
    if on_windows:
        install_file(build_dir / "tensorrt_llm/tensorrt_llm.dll", lib_dir / "tensorrt_llm.dll")
        install_file(build_dir / f"tensorrt_llm/thop/th_common.dll", lib_dir / "th_common.dll")
        install_file(
            build_dir / f"tensorrt_llm/plugins/nvinfer_plugin_tensorrt_llm.dll",
            lib_dir / "nvinfer_plugin_tensorrt_llm.dll",
        )
    else:
        install_file(build_dir / "tensorrt_llm/libtensorrt_llm.so", lib_dir / "libtensorrt_llm.so")
        install_file(build_dir / "tensorrt_llm/thop/libth_common.so", lib_dir / "libth_common.so")
        install_file(
        ...
        if os.path.exists(nixl_utils_dir / "libtensorrt_llm_nixl_wrapper.so"):
            install_file(
                nixl_utils_dir / "libtensorrt_llm_nixl_wrapper.so",
        ...
        agent_binding_so = list(nixl_utils_dir.glob("tensorrt_llm_transfer_agent_binding*.so"))
        if agent_binding_so:
            trtllm_dir = project_dir / "tensorrt_llm"
            install_file(agent_binding_so[0], trtllm_dir / agent_binding_so[0].name)
        if os.path.exists(
            build_dir
        ...
        install_file(
            build_dir / "tensorrt_llm/runtime/utils/libpg_utils.so", lib_dir / "libpg_utils.so"
        )
    ...
    if not on_windows:
        install_file(
            build_dir / "tensorrt_llm/executor_worker/executorWorker", bin_dir / "executorWorker"
        )
    ...
    if not cpp_only:
        ...
        with (build_dir / "tensorrt_llm" / "deep_ep" / "cuda_architectures.txt").open() as f:
        ...
        if deep_ep_cuda_architectures:
            ...
            install_file(
                build_dir / "tensorrt_llm/deep_ep/nvshmem-build/License.txt", lib_dir / "nvshmem"
            )
            install_file(
                build_dir / "tensorrt_llm/deep_ep/nvshmem-build/src/lib/nvshmem_bootstrap_uid.so.3",
                lib_dir / "nvshmem",
            )
            install_file(
                build_dir
        ...
        with (build_dir / "tensorrt_llm" / "flash_mla" / "cuda_architectures.txt").open() as f:
            flash_mla_cuda_architectures = f.read().strip().strip(";")
        if flash_mla_cuda_architectures:
            install_file(get_binding_lib("flash_mla", "flash_mla_cpp_tllm"), pkg_dir)
            install_tree(
    ...
    if not skip_building_wheel:
        ...
        if auto_attr.exists() and not (clean or first_build or configure_cmake):
            ...
            try:
                ...
                print(
                    f"Warning: Attribution generation step failed with error: {e}", file=sys.stderr
                )
                print(
        ...
        if auto_attr.exists():
            ...
            print(f"Copied auto-generated attributions to {project_dir / 'ATTRIBUTIONS.md'}")
...
def add_arguments(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--clean", "-c", action="store_true", help="Clean the build directory before building"
    )
    parser.add_argument(
        "--clean_wheel",
        action="store_true",
    ...
    parser.add_argument(
        "--configure_cmake", action="store_true", help="Always configure cmake before building"
    )
    parser.add_argument(
        "--use_ccache",
        default=False,
    ...
    parser.add_argument(
        ...
        nargs="?",
        help="Number of parallel jobs for compilation (default: number of CPUs available to this process, respecting affinity)",
    )
    parser.add_argument(
        "--cpp_only",
        "-l",
    ...
    parser.add_argument("--nccl_root", help="Directory containing NCCL headers and libraries")
    parser.add_argument("--nixl_root", help="Directory containing NIXL headers and libraries")
    parser.add_argument(
        "--mooncake_root",
        help="Directory containing Mooncake transfer engine headers and libraries",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dist_dir", type=Path, help="Directory where Python wheels are built (default: build/)"
    )
    parser.add_argument(
        "--skip_building_wheel",
        "-s",
    ...
    parser.add_argument(
        "--benchmarks", action="store_true", help="Build the benchmarks for the C++ runtime"
    )
    parser.add_argument(
        "--micro_benchmarks",
        action="store_true",
    ...
    parser.add_argument("--nvtx", action="store_true", help="Enable NVTX profiling features")
    parser.add_argument("--skip-stubs", action="store_true", help="Skip building Python type stubs")
    parser.add_argument("--generate_fmha", action="store_true", help="Generate the FMHA CUDA files")
    parser.add_argument(
        "--no-venv",
        action="store_true",
    ...
    parser.add_argument(
        "--mypyc", action="store_true", help="Compile kv_cache_manager_v2 with mypyc"
    )
    parser.add_argument(
        "--require_dynamic_attributions",
        action="store_true",
        help="Fail the build if attribution generation fails",
    )
    parser.add_argument(
        "--only_run_setup", action="store_true", help="Only run the setup steps and exit"
    )
